...
/**
 * Recursively find and move 'classes' and 'interfaces' directories up to their nearest parent
 * @param {string} currentDir - The current directory to process
 * @param {string} targetParentDir - The target parent directory where classes/interfaces should be moved
 */
async function organiseTypeDocRecursively(currentDir, targetParentDir) {
  try {
    ...
    for (const entry of entries) {
      ...
      // If this is a classes or interfaces directory, move it up to the target parent
      if (entry.name === 'classes' || entry.name === 'interfaces') {
        const targetPath = path.join(targetParentDir, entry.name);

        try {
          if (existsSync(targetPath)) {
            // If target exists, move contents instead of the directory
            const sourceContents = await fs.readdir(entryPath, {
              withFileTypes: true,
            });
            for (const item of sourceContents) {
              const sourceItemPath = path.join(entryPath, item.name);
              const targetItemPath = path.join(targetPath, item.name);

              // Handle name conflicts by adding a suffix
              let finalTargetPath = targetItemPath;
              let counter = 1;
              ...
              await fs.rename(sourceItemPath, finalTargetPath);
            }
            // Remove the now-empty source directory
            await fs.rmdir(entryPath);
          } else {
            // If target doesn't exist, move the entire directory
            await fs.rename(entryPath, targetPath);
          }

          console.log(`Moved: ${entryPath} → ${targetPath}`);
        } catch (err) {
          console.error(`Error moving ${entryPath} to ${targetPath}:`, err);
        }
      } else {
        // Recursively process subdirectories, but determine the appropriate target parent
        // If this directory might contain classes/interfaces, use the current targetParentDir
        // Otherwise, use this directory as the new target parent for its children
        const subEntries = await fs.readdir(entryPath, { withFileTypes: true });
        const hasClassesOrInterfaces = subEntries.some(
          (subEntry) =>
            isADirectory(subEntry) &&
            (subEntry.name === 'classes' || subEntry.name === 'interfaces'),
        );

        const nextTargetParent = hasClassesOrInterfaces
          ? targetParentDir
          : entryPath;
        await organiseTypeDocRecursively(entryPath, nextTargetParent);

        // After processing, try to remove the directory if it's empty
        try {
